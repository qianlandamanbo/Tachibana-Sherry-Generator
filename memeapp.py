...
def tk_periodic(interval_ms=1000):
    """
    定时装饰器。
    注意：被装饰的方法所属的类，必须有 self.root 属性。
    """
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            # 如果函数返回False，则停止定时，否则继续
            if should_continue is not False:
                # 保存timer_id到self上,防止垃圾回收,也可以用于取消
...
class MemeApp:
    ...
    def _setup_classic_mode(self):
        ...
        self.btn_color = tk.Button(
            group_style, text="点击修改文字颜色", bg="white", command=self._choose_color
        )
        ...
        btn_add_bg = ttk.Button(
            button_frame, text="+ 添加新背景", command=self._add_background
        )
        ...
        self.btn_save = ttk.Button(
            left_frame, text="保存图片 (Save)", command=self._save_image
        )
        # 设置 padding 把它撑大一点
    ...
    def _setup_listener_mode(self):
        ...
        self.btn_listen = ttk.Button(
            control_frame, text="开始监听", command=self._toggle_listener
        )
        ...
        self.check_outline_listener = ttk.Checkbutton(
            frame_sliders, text="启用描边", variable=self.var_use_outline
        )
        ...
        # 添加背景按钮
        btn_add_bg = ttk.Button(
            group_res, text="+ 添加新背景", command=self._add_background
        )
        ...
        self.btn_test = ttk.Button(
            btn_frame, text="测试生成图片", command=self._test_generate
        )
        self.btn_test.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        self.btn_update_preview = ttk.Button(
            btn_frame, text="更新预览", command=self._update_listener_preview
        )
    ...
    def _add_background(self):
        ...
        if files:
            try:
                ...
                self.log(f"✅已添加{len(self.selected_files)}个图片")
                # 重新加载资源列表
                self._load_resources()
    ...
    def _show_exit_dialog(self):
        ...
        ttk.Label(dialog, text="请选择退出方式:", font=("Microsoft YaHei", 10)).pack(
            pady=20
        )
        ...
        ttk.Button(btn_frame, text="直接退出", command=lambda: set_choice("exit")).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            btn_frame, text="后台运行", command=lambda: set_choice("background")
