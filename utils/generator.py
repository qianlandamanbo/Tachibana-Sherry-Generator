...
from PIL import Image, ImageDraw, ImageFont  # 导入 Pillow 库，用于强大的图像处理
...
class ImageGenerator:
    ...
    def render_image(self, settings):
        ...
        if not os.path.isfile(font_path): # 判断是否是有效文件
            return
        # 获取用户设置的最大字号，默认 100
        ...
        while current_size >= min_size:
            try:
                ...
                # 调用辅助函数计算换行和高度
                lines, h, line_h = self._calculate_wrapped_text(
                    draw, text, font, draw_area_w
                )
